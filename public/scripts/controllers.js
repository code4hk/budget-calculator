...
define(
  ["angular", "hkTaxCal", "taxCalculator.translations.en"],
  function(angular, calculator) {
        ...
        function($scope, $window, $location, $anchorScroll, $timeout) {
          ...
          if ($window._gaq) {
            $scope.$on('$viewContentLoaded', function(event) {
              var code4hkLink = angular.element(document.querySelector(
                '#code4hk-badge-link'));
              code4hkLink.on('click', function() {
                $window._gaq.push(['_trackEvent', 'link', 'click',
                  'code4hk-badge-link'
                ]);
              });
            });
          ...
          var allowancesItems = ["basic", "married", "child", "bornChild",
            "dependentSiblings", "dependent60Parents",
            "dependent60ParentsResidedWith", "dependent55Parents",
            "dependent55ParentsResidedWith", "singleParent",
            "disabledDependent"
          ];
          var deductionsItems = ["outgoingExpenses", "selfEduExpenses",
            "donations", "mpf", "others"
          ];
          ...
          var calculateLiving = function(year) {
            ...
            livingItems.map(function(item) {
              ...
              if (typeof($scope[year][item + 'Formula']) ===
                'function') {
                $scope[year][item + 'Tax'] = $scope[year][item +
                  'Formula'
                ](($scope.living[item + 'Count'] | 0));
              } else {
                $scope[year][item + 'Tax'] = ($scope.living[item +
                  'Count'] | 0) * $scope[year][item + 'Formula'];
              }
          ...
          var calculateIncomeTax = function(year) {
            ...
            var taxPayable = calculator.calculateTax(year, income,
              deduction, totalAllowances);
          ...
          var calculateReductions = function(year, taxPayable) {
            return $scope[year]['salary']['reductions'] = calculator.calReductions(
              year, taxPayable);
          };
          var calculateDeductions = function(year) {
            deductionsItems.map(function(item) {
              $scope[year]['salary']['deductionsEntitled'][item] =
                $scope.salaryTaxInfo['deductions'][item + 'Count'] *
                1;
            });
            return $scope[year]['salary']['deductionsEntitled']['total'] =
              deductionsItems.reduce(function(p, item) {
                return (p | 0) + ($scope[year]['salary'][
                  'deductionsEntitled'
                ][item] | 0)
              }, 0);
            // return
          ...
          var calculateAllowances = function(year) {
            allowancesItems.map(function(item) {
              $scope[year]['salary']['allowancesEntitled'][item] =
                calculator.calAllowances(year, item, $scope.salaryTaxInfo
                  .allowances[item + 'Count']);
            });


            $scope[year]['salary']['allowancesEntitled']['totalChild'] =
            ...
            $scope[year]['salary']['allowancesEntitled']['basicPerStatus'] =
              Math.max($scope[year]['salary']['allowancesEntitled'][
                'basic'
              ], $scope[year]['salary']['allowancesEntitled']['married']);


            return $scope[year]['salary']['allowancesEntitled']['total'] =
              allowancesItems.reduce(function(p, item) {
                return (p | 0) + ($scope[year]['salary'][
                  'allowancesEntitled'
                ][item] | 0);
              }, 0);
          ...
          var toggleAllowancesForMarried = function(isMarried) {
            if (isMarried) {
              ...
              $scope.salaryTaxInfo['allowances']['isSingleParent'] =
                false;
            } else {
          ...
          $scope.$watch('salaryTaxInfo', function(newVal, oldVal) {
            ...
            $scope.salaryTaxInfo['allowances']['singleParentCount'] =
              $scope.salaryTaxInfo['allowances']['isSingleParent'] ===
              "true" ? 1 : 0;
            // salary
