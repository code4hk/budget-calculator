...
define([], function() {
  ...
  _calculator.calculateTax = function(year, income, deduction,
    totalAllowances) {
    var deductedIncome = income - deduction;
    deductedIncome = Math.max(deductedIncome, 0);
    var netChargeableIncome = Math.max(deductedIncome - totalAllowances,
      0);
    netChargeableIncome = netChargeableIncome > 0 ? netChargeableIncome :
      0;
    var taxProgressive = _calculator.calProgressive(netChargeableIncome,
      year);
    var taxStandardRate = _calculator.calStandardRate(deductedIncome,
      year);
    return taxProgressive <= taxStandardRate ? taxProgressive :
      taxStandardRate;
  };
